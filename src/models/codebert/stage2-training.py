"""
Stage 2: Train CodeBERT for Dockerfile rule prediction
- Target: label_rule
...
DATA_DIR = "data/codebert/tokenized_chunks-stage2"
OUTPUT_DIR = "saved_models/stage2_codebert"
...
def get_loader(split):
    ...
    if len(chunk_files) == 0:
        raise RuntimeError(f"[ERROR] No chunk files found in {split_dir}")
...
num_labels = train_df["label_rule"].nunique()

print(f"[INFO] Number of label_rule classes: {num_labels}")
...
print(f"[INFO] Stage 2 CodeBERT training completed.")
print(f"[INFO] Model + tokenizer saved to {OUTPUT_DIR}")
