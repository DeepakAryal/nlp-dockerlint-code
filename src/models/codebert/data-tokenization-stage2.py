...
- Uses CodeBERT (microsoft/codebert-base)
...
CHUNK_SIZE = 5000
DATA_DIR = "data"
OUTPUT_DIR = "data/codebert/tokenized_chunks-stage2"
RANDOM_STATE = 42
...
def tokenize_split(split_name, csv_file, label_encoder=None):
    ...
    for df_chunk in tqdm(df_iter, desc=f"Tokenizing {split_name} chunks"):
        ...
        chunk_file = os.path.join(
            split_output_dir,
            f"{split_name}_chunk_{chunk_count}.pt"
        )
        torch.save(dataset, chunk_file, pickle_protocol=4)

        print(
            f"[INFO] Saved chunk {chunk_count} "
            f"({len(df_chunk)} rows) to {chunk_file}"
        )
