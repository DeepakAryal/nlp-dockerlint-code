...
from src.linting.henkel.dockerfile_linter import HadolintLinter
from src.linting.henkel.excel_saver import ExcelSaver

def main():
    HENKEL_DIR = "dockerfiles/henkel"
    OUTPUT_FILE = "dockerfile_analysis.xlsx"
    MAX_FILES = 5
    ...
    henkel_path = Path(HENKEL_DIR)
    if not henkel_path.exists():
        print(f"[ERROR] Directory not found: {HENKEL_DIR}")
        return
    
    dockerfiles = list(henkel_path.glob("*.Dockerfile"))
    print(f"[INFO] Found {len(dockerfiles)} Dockerfiles in {HENKEL_DIR}")
